import {
  CARD_SHOP_FREE_PACK_COOLDOWN_MS,
  CARD_SHOP_STARTER_CREDITS,
  cardShopProducts,
} from "@/data/card-shop";
...
export const CARD_SHOP_VERSION = 2;
...
const rarityRank: Record<CustomizationRarity, number> = {
  common: 0,
  uncommon: 1,
  rare: 2,
  epic: 3,
...
const rarityRecycle: Record<CustomizationRarity, number> = {
  common: 8,
  uncommon: 14,
  rare: 28,
  epic: 65,
...
const variantMultiplier: Record<LokDexCardVariant, number> = {
  standard: 1,
  foil: 1.5,
  holo: 2.5,
  negative: 3,
...
export function createCardShopState(): CardShopState {
  return {
    ...
    creditsFromGameplay: 0,
    creditsFromCompanions: 0,
    cardPackVouchers: 0,
...
export function normalizeCardShopState(
  input?: Partial<CardShopState> | null,
): CardShopState {
  ...
  return {
    ...
    credits: Math.max(
      0,
      Number.isFinite(input?.credits) ? Number(input!.credits) : 0,
    ),
    lifetimeCreditsEarned: Math.max(
      0,
      Number.isFinite(input?.lifetimeCreditsEarned)
        ? Number(input!.lifetimeCreditsEarned)
        : 0,
    ),
    lifetimeCreditsSpent: Math.max(
      0,
      Number.isFinite(input?.lifetimeCreditsSpent)
        ? Number(input!.lifetimeCreditsSpent)
        : 0,
    ),
    packsOpened: Math.max(
      0,
      Math.floor(
        Number.isFinite(input?.packsOpened) ? Number(input!.packsOpened) : 0,
      ),
    ),
    cardsPulled: Math.max(
      0,
      Math.floor(
        Number.isFinite(input?.cardsPulled) ? Number(input!.cardsPulled) : 0,
      ),
    ),
    cardsRecycled: Math.max(
      0,
      Math.floor(
        Number.isFinite(input?.cardsRecycled)
          ? Number(input!.cardsRecycled)
          : 0,
      ),
    ),
    creditsFromRecycling: Math.max(
      0,
      Number.isFinite(input?.creditsFromRecycling)
        ? Number(input!.creditsFromRecycling)
        : 0,
    ),
    creditsFromCollectionRewards: Math.max(
      0,
      Number.isFinite(input?.creditsFromCollectionRewards)
        ? Number(input!.creditsFromCollectionRewards)
        : 0,
    ),
    creditsFromGameplay: Math.max(
      0,
      Number.isFinite(input?.creditsFromGameplay)
        ? Number(input!.creditsFromGameplay)
        : 0,
    ),
    creditsFromCompanions: Math.max(
      0,
      Number.isFinite(input?.creditsFromCompanions)
        ? Number(input!.creditsFromCompanions)
        : 0,
    ),
    cardPackVouchers: Math.max(
      0,
      Math.floor(
        Number.isFinite(input?.cardPackVouchers)
          ? Number(input!.cardPackVouchers)
          : 0,
      ),
    ),
    claimedCollectionMilestones: Array.isArray(
      input?.claimedCollectionMilestones,
    )
      ? [
          ...new Set(
            input!.claimedCollectionMilestones
      ...
      : [],
    ownedDeckBlueprintIds: Array.isArray(input?.ownedDeckBlueprintIds)
      ? [
          ...new Set(
            input!.ownedDeckBlueprintIds.filter(
              (id): id is string => typeof id === "string",
            ),
          ),
        ]
      : [],
    lastFreePackAt: Math.max(
      0,
      Number.isFinite(input?.lastFreePackAt)
        ? Number(input!.lastFreePackAt)
        : 0,
    ),
    freePacksClaimed: Math.max(
      0,
      Math.floor(
        Number.isFinite(input?.freePacksClaimed)
          ? Number(input!.freePacksClaimed)
          : 0,
      ),
    ),
    starterGrantClaimed: !!input?.starterGrantClaimed,
    recentPulls: Array.isArray(input?.recentPulls)
      ? input!.recentPulls.slice(0, 24)
      : [],
    recentCreditRewards: Array.isArray(input?.recentCreditRewards)
      ? input!.recentCreditRewards.slice(0, 16)
      : [],
    gameplayLedgers: Array.isArray(input?.gameplayLedgers)
      ? input!.gameplayLedgers.slice(-8)
      : [],
    activeCompanionQuest: input?.activeCompanionQuest ?? null,
    completedCompanionQuestIds: Array.isArray(input?.completedCompanionQuestIds)
      ? [
          ...new Set(
            input!.completedCompanionQuestIds.filter(
              (id): id is string => typeof id === "string",
            ),
          ),
        ].slice(-80)
...
export function loadCardShopState() {
  ...
  try {
    ...
    return raw
      ? normalizeCardShopState(JSON.parse(raw))
      : createCardShopState();
  } catch {
    return createCardShopState();
  }
}
...
export function saveCardShopState(input: CardShopState) {
  ...
  if (typeof window !== "undefined") {
    try {
      localStorage.setItem(CARD_SHOP_KEY, JSON.stringify(next));
    } catch {}
  }
  return next;
}
...
export function ensureCardShopStarterGrant(input: CardShopState) {
  ...
  return {
    ...
    lifetimeCreditsEarned:
      state.lifetimeCreditsEarned + CARD_SHOP_STARTER_CREDITS,
    starterGrantClaimed: true,
  };
}

export function applyCardCollectionMilestones(
  shopInput: CardShopState,
  collectionInput: LokDexCollection,
) {
  ...
  const newlyClaimed = CARD_COLLECTION_REWARDS.filter(
    (reward) =>
      discovered >= reward.discovered &&
      !shop.claimedCollectionMilestones.includes(reward.discovered),
  );
  if (!newlyClaimed.length)
    return { shop, creditsAwarded: 0, milestones: [] as number[] };
  const creditsAwarded = newlyClaimed.reduce(
    (sum, reward) => sum + reward.credits,
    0,
  );
  shop = normalizeCardShopState({
    ...
    creditsFromCollectionRewards:
      shop.creditsFromCollectionRewards + creditsAwarded,
    claimedCollectionMilestones: [
      ...shop.claimedCollectionMilestones,
      ...newlyClaimed.map((reward) => reward.discovered),
    ],
  });
  return {
    shop,
    creditsAwarded,
    milestones: newlyClaimed.map((reward) => reward.discovered),
  };
}

function weightedCharacter(product: CardShopProduct, guarantee = false) {
  let pool = lokDexEntries.filter(
    (entry) => entry.acquisition.includes("pack") && entry.rarity !== "secret",
  );
  ...
  const weighted = pool.flatMap((entry) => {
    ...
    else if (entry.rarity === "epic") weight += 0.65;
    else if (entry.rarity === "legendary") weight += 0.18;
    else if (entry.rarity === "mythic") weight += 0.025;
    return Array.from(
...
function editionForPull(product: CardShopProduct, guarantee = false) {
  ...
  const chance = Math.max(
    0,
    Math.min(1, product.guarantee?.editionChance ?? 0),
  );
  if (!ids.length || Math.random() >= chance) return null;
  let editions = ids
    .map((id) => lokDexEditionById(id))
  ...
  if (guarantee && product.guarantee?.minRarity) {
    ...
    const filtered = editions.filter((edition) => {
      const base = lokDexEntries.find(
        (entry) => entry.id === edition.baseCharacterId,
      );
...
function rollVariant(boost = 1): LokDexCardVariant {
  ...
  const gold = 0.002 * boost;
  const glitch = 0.005 * boost;
  const negative = 0.009 * boost;
  const holo = 0.025 * boost;
  const foil = 0.07 * boost;
...
function pullProduct(
  ...
  for (let index = 0; index < product.cardCount; index += 1) {
    ...
    const character = edition
      ? (lokDexEntries.find((entry) => entry.id === edition.baseCharacterId) ??
        weightedCharacter(product, guarantee))
      : fixedId
        ? (lokDexEntries.find((entry) => entry.id === fixedId) ??
          weightedCharacter(product, guarantee))
        : weightedCharacter(product, guarantee);
    ...
    pulls.push({
      ...
      isNewCharacter: !wasDiscovered,
    });
  }
  return { collection, pulls };
}

export function purchaseCardProduct(
  shopInput: CardShopState,
  collectionInput: LokDexCollection,
  productId: string,
) {
  ...
  if (!product || shop.credits < product.priceCredits)
    return {
      success: false as const,
      shop,
      collection: normalizeLokDexCollection(collectionInput),
      pulls: [] as CardShopPull[],
    };
  const opened = pullProduct(collectionInput, product);
  const discoveryBonus = opened.pulls
    .filter((pull) => pull.isNewCharacter)
    .reduce((sum, pull) => {
  ...
  shop = normalizeCardShopState({
    ...
    ownedDeckBlueprintIds: product.deckBlueprintId
      ? [...shop.ownedDeckBlueprintIds, product.deckBlueprintId]
      : shop.ownedDeckBlueprintIds,
  ...
  return {
    ...
    pulls: opened.pulls,
    discoveryBonus,
    milestoneBonus: milestone.creditsAwarded,
...
export function freePackReady(shopInput: CardShopState, now = Date.now()) {
  ...
  return (
    shop.lastFreePackAt === 0 ||
    now - shop.lastFreePackAt >= CARD_SHOP_FREE_PACK_COOLDOWN_MS
  );
}

export function freePackRemainingMs(
  shopInput: CardShopState,
  now = Date.now(),
) {
  const shop = normalizeCardShopState(shopInput);
  return freePackReady(shop, now)
    ? 0
    : Math.max(
        0,
        CARD_SHOP_FREE_PACK_COOLDOWN_MS - (now - shop.lastFreePackAt),
      );
}

export function claimFreeCardPack(
  shopInput: CardShopState,
  collectionInput: LokDexCollection,
  now = Date.now(),
) {
  let shop = ensureCardShopStarterGrant(shopInput);
  if (!freePackReady(shop, now))
    return {
      success: false as const,
      shop,
      collection: normalizeLokDexCollection(collectionInput),
  ...
  const opened = pullProduct(collectionInput, {
    ...product,
    cardCount: 3,
    guarantee: { ...product.guarantee, minRarity: "uncommon" },
  });
  shop = normalizeCardShopState({
    ...shop,
    lastFreePackAt: now,
...
export function claimCompanionCardPack(
  shopInput: CardShopState,
  collectionInput: LokDexCollection,
) {
  let shop = ensureCardShopStarterGrant(shopInput);
  if (shop.cardPackVouchers < 1)
    return {
      success: false as const,
      shop,
      collection: normalizeLokDexCollection(collectionInput),
...
export function recycleValue(card: LokDexOwnedCard) {
  ...
  return character
    ? Math.max(
        1,
        Math.round(
          rarityRecycle[character.rarity] *
            variantMultiplier[card.variant] *
            editionMultiplier,
        ),
      )
    : 1;
...
export function recycleDuplicateCard(
  shopInput: CardShopState,
  collectionInput: LokDexCollection,
  characterId: string,
) {
  ...
  if (copies.length < 2)
    return { success: false as const, shop, collection, creditsGained: 0 };
  const recyclable =
    copies.find((card) => !card.favorite) ?? copies[copies.length - 1];
  const creditsGained = recycleValue(recyclable);
  const nextCollection = {
    ...collection,
    cards: collection.cards.filter(
      (card) => card.instanceId !== recyclable.instanceId,
    ),
  };
