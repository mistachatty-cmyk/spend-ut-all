...
export function newGame(scenarioId: ScenarioId, mode: GameState['mode'], riskMode = false): GameState {
  ...
  const now = Date.now();
  return { started: true, scenarioId, mode, cash: scenario.startingCash, totalSpent: 0, totalSold: 0, lifetimeIncome: 0, owned: {}, incomeStreams: {}, businesses: {}, cityEconomy: createCityEconomyState(now), houseLevel: 0, townLevel: 0, regionLevel: 0, upgrades: {}, citySpecialization: null, activeEventId: null, eventEndsAt: 0, nextEventAt: now + EVENT_INTERVAL_MS, lastOfflineIncome: 0, riskMode, runStatus: 'active', bankruptcyDeadline: 0, peakCash: scenario.startingCash, peakNetWorth: scenario.startingCash, lokTokens: 0, lokProgressMs: 0, theme: 'light', createdAt: now, updatedAt: now };
}
...
export function advance(state: GameState, deltaMs: number): GameState {
  ...
  let next = { ...safeState, cash: safeState.cash + income, lifetimeIncome: safeState.lifetimeIncome + Math.max(0, income), lokTokens: lok.balance, lokProgressMs: lok.progressMs, lastOfflineIncome: 0, updatedAt: now };
