...
export function maxAffordableQuantity(state: GameState, item: GameItem) { if ((item.unlockSpent ?? 0) > state.totalSpent || state.cash < itemUnitPrice(item, state.owned[item.id] ?? 0)) return 0; const owned = state.owned[item.id] ?? 0; if (item.growthRate === 1) return Math.floor(state.cash / item.basePrice); const factor = 1 + (state.cash * (item.growthRate - 1)) / (item.basePrice * Math.pow(item.growthRate, owned)); return Math.max(0, Math.floor(Math.log(factor) / Math.log(item.growthRate))); }
...
export function passiveCashPerSecond(state: GameState) { return grossIncomePerSecond(state) - upkeepPerSecond(state); }
...
export function canBuyItem(state: GameState, item: GameItem, quantity = 1) { if ((item.unlockSpent ?? 0) > state.totalSpent) return false; return state.cash >= itemBulkPrice(item, state.owned[item.id] ?? 0, quantity); }
export function buyItem(state: GameState, item: GameItem, quantity = 1): GameState { const owned = state.owned[item.id] ?? 0; const price = itemBulkPrice(item, owned, quantity); if (quantity <= 0 || (item.unlockSpent ?? 0) > state.totalSpent || state.cash < price) return state; return { ...state, cash: state.cash - price, totalSpent: state.totalSpent + price, owned: { ...state.owned, [item.id]: owned + quantity }, updatedAt: Date.now() }; }
export function sellItem(state: GameState, item: GameItem, quantity = 1): GameState { const owned = state.owned[item.id] ?? 0; const sold = Math.min(Math.max(0, quantity), owned); if (!sold) return state; const originalPrice = itemBulkPrice(item, owned - sold, sold); const refund = originalPrice * 0.7; return { ...state, cash: state.cash + refund, totalSold: state.totalSold + refund, owned: { ...state.owned, [item.id]: owned - sold }, updatedAt: Date.now() }; }
export function upgradeHouse(state: GameState): GameState { const next = houseTiers.find((tier) => tier.level === state.houseLevel + 1); if (!next || state.cash < next.cost || netWorth(state) < next.requiredNetWorth) return state; return { ...state, cash: state.cash - next.cost, totalSpent: state.totalSpent + next.cost, houseLevel: next.level, updatedAt: Date.now() }; }
export function upgradeTown(state: GameState): GameState { if (state.houseLevel < 5) return state; const next = townTiers.find((tier) => tier.level === state.townLevel + 1); if (!next || state.cash < next.cost || netWorth(state) < next.requiredNetWorth) return state; return { ...state, cash: state.cash - next.cost, totalSpent: state.totalSpent + next.cost, townLevel: next.level, updatedAt: Date.now() }; }
export function upgradeRegion(state: GameState): GameState { if (state.townLevel < 5) return state; const next = regionTiers.find((tier) => tier.level === state.regionLevel + 1); if (!next || state.cash < next.cost || netWorth(state) < next.requiredNetWorth) return state; return { ...state, cash: state.cash - next.cost, totalSpent: state.totalSpent + next.cost, regionLevel: next.level, updatedAt: Date.now() }; }
...
export function canBuyUpgrade(state: GameState, upgrade: EmpireUpgrade) { const level = state.upgrades?.[upgrade.id] ?? 0; if (level >= upgrade.maxLevel || (upgrade.requiredTownLevel ?? 0) > state.townLevel || (upgrade.requiredRegionLevel ?? 0) > state.regionLevel) return false; return state.cash >= upgradeCost(state, upgrade); }
export function buyUpgrade(state: GameState, upgrade: EmpireUpgrade): GameState { if (!canBuyUpgrade(state, upgrade)) return state; const level = state.upgrades?.[upgrade.id] ?? 0, cost = upgradeCost(state, upgrade); return { ...state, cash: state.cash - cost, totalSpent: state.totalSpent + cost, upgrades: { ...state.upgrades, [upgrade.id]: level + 1 }, updatedAt: Date.now() }; }
export function chooseCitySpecialization(state: GameState, specialization: CitySpecializationId): GameState { if (state.townLevel < 4 || state.citySpecialization) return state; return { ...state, citySpecialization: specialization, updatedAt: Date.now() }; }
