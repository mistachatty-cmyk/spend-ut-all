...
export function maxAffordableQuantity(state: GameState, item: GameItem) { if ((item.unlockSpent ?? 0) > state.totalSpent) return 0; const owned = state.owned[item.id] ?? 0; const budget = state.riskMode ? availableBuyingPower(state, netWorth(state)) : Math.max(0, state.cash); if (budget < itemUnitPrice(item, owned)) return 0; if (item.growthRate === 1) return Math.floor(budget / item.basePrice); const factor = 1 + (budget * (item.growthRate - 1)) / (item.basePrice * Math.pow(item.growthRate, owned)); return Math.max(0, Math.floor(Math.log(factor) / Math.log(item.growthRate))); }
...
export function advance(state: GameState, deltaMs: number): GameState {
  ...
  if (timeTick.completedActivityId) activityIncome = activities.find((entry) => entry.id === timeTick.completedActivityId)?.income ?? 0;
