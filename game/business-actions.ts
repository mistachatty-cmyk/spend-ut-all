...
import { freelanceBusinessReadiness } from './systems/freelance';
...
export function businessFoundingCost(state: GameState, definition: BusinessDefinition) {
  if (!state.life.enabled) return definition.foundingCost;
  const readiness = freelanceBusinessReadiness(state, definition.id);
  return Math.max(1, Math.round(definition.foundingCost * (1 - readiness.discount)));
}
...
export function foundBusiness(state: GameState, definition: BusinessDefinition): GameState {
  ...
  if (current.founded || state.cash < foundingCost || !businessUnlocked(state, definition)) return state;

  const employees = definition.employeesPerLocation;
  let life = state.life;
  if (state.life.enabled) {
    life = gainLifeSkillXp(
      life,
      'management',
      Math.max(8, Math.min(45, Math.round(Math.log10(Math.max(10, foundingCost)) * 5))),
    );
  ...
  return {
    ...
    totalSpent: state.totalSpent + foundingCost,
    businesses: {
      ...state.businesses,
      [definition.id]: { ...current, founded: true, locations: 1, employees },
    },
    updatedAt: Date.now(),
  };
}
  ...
  return {
    ...
    businesses: {
      ...
      [definition.id]: {
        ...
        employees: current.employees + definition.employeesPerLocation,
      },
    },
    updatedAt: Date.now(),
  };
...
export function hireEmployees(state: GameState, definition: BusinessDefinition, quantity: number): GameState {
  ...
  return {
    ...state,
    businesses: { ...state.businesses, [definition.id]: { ...current, employees: nextEmployees } },
    updatedAt: Date.now(),
  };
}
...
export function reduceEmployees(state: GameState, definition: BusinessDefinition, quantity: number): GameState {
  ...
  return {
    ...
    businesses: {
      ...state.businesses,
      [definition.id]: { ...current, employees: Math.max(0, current.employees - quantity) },
    },
    updatedAt: Date.now(),
  };
}
...
export function upgradeBusinessHq(state: GameState, definition: BusinessDefinition): GameState {
  ...
  return {
    ...
    cash: state.cash - cost,
    totalSpent: state.totalSpent + cost,
    businesses: { ...state.businesses, [definition.id]: { ...current, hqLevel: current.hqLevel + 1 } },
    updatedAt: Date.now(),
  };
}

export function upgradeBusinessManagement(
  state: GameState,
  definition: BusinessDefinition,
  kind: 'marketing' | 'operations' | 'quality',
): GameState {
  const current = currentBusiness(state, definition);
  if (!current.founded) return state;
  ...
  return {
    ...
    cash: state.cash - cost,
    totalSpent: state.totalSpent + cost,
    businesses: { ...state.businesses, [definition.id]: { ...current, [key]: current[key] + 1 } },
    updatedAt: Date.now(),
  };
}
...
export function businessSnapshot(state: GameState, definition: BusinessDefinition) {
  ...
  return {
    business,
    economics: businessEconomics(definition, business, {
