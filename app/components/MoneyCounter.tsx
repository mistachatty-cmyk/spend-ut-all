...
export function MoneyCounter({ state, income }: { state: GameState; income: number }) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem(HUD_PREFS_KEY);
      if (saved) setPrefs({ ...defaultPrefs, ...JSON.parse(saved) });
    } catch {}
  }, []);
  useEffect(() => {
    try { localStorage.setItem(HUD_PREFS_KEY, JSON.stringify(prefs)); } catch {}
  }, [prefs]);
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setClockTick((value) => value + 1), 47);
  ...
  return <div className={`live-money live-money-${status}`}>
    ...
    <section className="counter-hud" aria-label="Run counters">
      ...
      {prefs.showRunClock ? <div className="counter-card run-counter">
        <span className="counter-label">ACTIVE PLAYTIME</span>
        <div className="counter-value"><b>{formatRunClock(liveActivePlayMs)}</b><small>real active time</small></div>
        <div className="counter-unit-row">
          <span>{parts.days}d</span><span>{parts.hours}h</span><span>{parts.minutes}m</span><span>{parts.seconds}s</span>
          {prefs.showMilliseconds ? <span>{String(parts.milliseconds).padStart(3, '0')}ms</span> : null}
        </div>
      </div> : null}

      {prefs.showGameDay ? <div className="counter-card day-counter">
        <span className="counter-label">GAME WORLD</span>
    ...
    <div className="counter-actions">
      <button type="button" onClick={() => setShowCounterMenu((value) => !value)} aria-expanded={showCounterMenu}>Counters ⚙</button>
      {showCounterMenu ? <div className="counter-menu">
        <label><input type="checkbox" checked={prefs.showLok} onChange={() => patchPref('showLok')} /> LOK</label>
