...
import { borrowFromProduct, canBorrowProduct, collateralCandidates, consolidateDebts, consolidationQuote, estimatedBorrowAmount, financeNextHome, mortgageQuote, refinanceDebt, refinanceQuote, repayDebt, repayMinimum, setDebtAutopay, setDebtSystemEnabled, settleCourtCase } from '@/game/debt-actions';
...
export function DebtView({ state, setState }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState | null>> }) {
  ...
  return <section className="debt-shell">
    ...
    <section className="panel court-panel"><div className="section-heading"><div><span className="eyebrow">CIVIL COURT</span><h2>Defaults can become legal cases</h2></div><span>Unsecured creditors sue. Secured lenders generally pursue pledged collateral first.</span></div>{cases.length ? <div className="court-list">{cases.map((court) => {
