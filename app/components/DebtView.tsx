...
export function DebtView({ state, setState }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState | null>> }) {
  ...
  const applyFinancialAction = (sourceElement: HTMLElement, action: (current: GameState) => GameState) => setState((current) => {
    if (!current) return current;
    const beforeCash = current.cash;
    const beforeDebt = debtSummary(normalizeDebtState(current.debt)).totalDebt;
    const next = action(current);
    const afterCash = next.cash;
    const afterDebt = debtSummary(normalizeDebtState(next.debt)).totalDebt;
    const cashDelta = afterCash - beforeCash;
    const debtDelta = afterDebt - beforeDebt;
    if (Math.abs(cashDelta) > .001) emitMicroMotion({ target:'cash', amount:cashDelta, displayText:`${cashDelta > 0 ? '+' : '-'}${money(Math.abs(cashDelta))}`, symbol:'$', tone:cashDelta > 0 ? 'positive' : 'negative', kind:'currency', sourceElement });
    if (Math.abs(debtDelta) > .001) emitMicroMotion({ target:'debt', amount:debtDelta, displayText:`${debtDelta > 0 ? '+' : '-'}${money(Math.abs(debtDelta))} debt`, symbol:'▥', tone:debtDelta > 0 ? 'debt' : 'reward', kind:'debt', sourceElement, delayMs:80 });
    return next;
  });
  ...
  return <section className="debt-shell">
    <section className="panel debt-hero">
      <div><span className="eyebrow">LEVERAGE & LIABILITY</span><h2 data-motion-target="debt">{money(summary.totalDebt)} owed</h2><p>Borrowing raises cash and liabilities together. Wealth goals use debt-adjusted net worth, so leverage can accelerate purchases without pretending borrowed money is profit.</p></div>
    ...
    <section className="panel court-panel"><div className="section-heading"><div><span className="eyebrow">CIVIL COURT</span><h2>Choose how to respond</h2></div><span>Fictional game simulation: settle, retain counsel, buy time, or convert eligible cases into a payment plan.</span></div>{cases.length ? <div className="court-list">{cases.map((court) => {
      ...
      return <article className={`court-card stage-${court.stage}`} key={court.id}><span className="court-icon">⚖️</span><div className="court-case-copy"><div className="court-case-title"><b>{court.plaintiff} v. Player</b><small>{court.stage.toUpperCase()} · claim {money(court.amountClaimed)}</small></div><div className="court-chips"><span>{representation === 'self' ? 'Self-represented' : representation === 'local-counsel' ? 'Local counsel' : 'Elite firm'}</span><span>{court.continuances ?? 0}/2 continuances</span>{(court.settlementDiscount ?? 0) > 0 ? <span>{Math.round((court.settlementDiscount ?? 0) * 100)}% settlement leverage</span> : null}</div><p>{court.stage === 'filed' ? `A collection case has been filed. Hearing scheduled ${gameDateLabel(court.nextEventGameMinute)}.` : court.stage === 'hearing' ? `The case is at hearing. Unresolved claims can become judgments around ${gameDateLabel(court.nextEventGameMinute)}.` : court.stage === 'judgment' ? `Judgment entered for ${money(court.judgmentAmount ?? court.amountClaimed)}. Later systems can add liens, garnishment, appeals, and restructuring.` : court.paymentPlan ? 'The lawsuit is resolved into a structured payment plan.' : 'This case has been resolved.'}</p>{court.lastAction ? <small className="court-last-action">Last action: {court.lastAction}</small> : null}<div className="court-strategy-actions">{representation === 'self' && localCounsel ? <button disabled={state.cash < localCounsel.fee} onClick={(event) => applyFinancialAction(event.currentTarget, (current) => hireCourtCounsel(current, court.id, 'local-counsel'))}>Local counsel · {money(localCounsel.fee)}</button> : null}{representation !== 'elite-firm' && eliteCounsel ? <button disabled={state.cash < eliteCounsel.fee} onClick={(event) => applyFinancialAction(event.currentTarget, (current) => hireCourtCounsel(current, court.id, 'elite-firm'))}>Elite firm · {money(eliteCounsel.fee)}</button> : null}{continuance ? <button disabled={state.cash < continuance.fee} onClick={(event) => applyFinancialAction(event.currentTarget, (current) => requestCourtContinuance(current, court.id))}>Continuance +{continuance.days}d · {money(continuance.fee)}</button> : null}{plan ? <button onClick={(event) => applyFinancialAction(event.currentTarget, (current) => acceptCourtPaymentPlan(current, court.id))}>Payment plan · ~{money(plan.minimum)}/mo</button> : null}{settlement ? <button disabled={!settlement.affordable} onClick={(event) => applyFinancialAction(event.currentTarget, (current) => settleCourtCaseNegotiated(current, court.id))}>Settle · {money(settlement.amount)}</button> : null}</div></div></article>;
