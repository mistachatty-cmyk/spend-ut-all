...
import { chooseLifeBackground, currentHousing, enableLifeRpg, lifeSkillProgress, moveToHousing } from '@/game/systems/life-progression';
import type { GameState } from '@/game/types';

const statLabels = [
  ['grit','Grit','Work stamina & persistence'],
  ['focus','Focus','Concentration & deliberate practice'],
  ['people','People','Communication & customer confidence'],
  ['knowledge','Knowledge','Technical & financial learning'],
  ['adaptability','Adaptability','Handling new work and changing conditions'],
] as const;
...
export function LifeRpgView({ state, setState }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState | null>> }) {
  ...
  if (!life.enabled) {
    return <section className="panel life-rpg-optin">
      <div><span className="eyebrow">OPTIONAL LIFE / RPG LAYER</span><h2>Turn the climb into a character build</h2><p>Enable skills, starting background, personal stats and realistic housing choices. Skills can unlock work, passive income and businesses. Leave it off and the core economy keeps its normal unlock rules.</p></div>
      <button className="primary" type="button" onClick={() => setState((current) => current ? { ...current, life: enableLifeRpg(current.life,current.time.gameMinute,current.time.settings.dayLengthMinutes), updatedAt:Date.now() } : current)}>Enable Life RPG</button>
    </section>;
  }
  ...
  return <section className="life-rpg-shell">
    ...
    <section className="panel life-profile-panel">
      <div className="life-profile-head"><div><span className="eyebrow">YOUR BUILD</span><h2>{lifeBackgrounds.find((entry)=>entry.id===life.backgroundId)?.name ?? 'Building your path'}</h2></div><span className="life-housing-chip">{housing.emoji} {housing.name}</span></div>
    ...
    <section className="panel life-skills-panel">
      ...
      <div className="life-skill-grid">{lifeSkills.map((skill)=>{const progress=lifeSkillProgress(life,skill.id);return <article key={skill.id}><header><span>{skill.emoji}</span><div><b>{skill.name}</b><small>{skill.description}</small></div><em>Lv {progress.level}</em></header><div className="life-xp-bar"><i><span style={{width:`${Math.round(progress.progress*100)}%`}} /></i><small>{Math.round(progress.xp).toLocaleString()} XP{progress.level<10 ? ` · ${Math.max(0,progress.next-progress.xp).toFixed(0)} to next` : ' · MAX'}</small></div><p>{skill.examples}</p></article>})}</div>
    </section>
    ...
    <section className="panel life-housing-panel">
      ...
      <p className="muted">Stay with someone, use temporary housing, rent, lease or buy. Recurring housing costs advance with simulated game days; Classic/non-time play is not quietly charged while the clock is disabled.</p>
