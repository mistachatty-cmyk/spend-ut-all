...
export function PixelPetSprite({ petId, mood = 'idle', silhouette = false, size = 52 }: { petId: string; mood?: PetMood; silhouette?: boolean; size?: number }) {
  ...
  const animation = sprite.animation[mood];
  const columns = sprite.grid[0]?.length ?? 10;
  const cell = size / columns;

  return <span className={`pixel-pet pixel-pet-${animation} ${silhouette ? 'pixel-pet-silhouette' : ''}`} style={{ width: size, height: size, gridTemplateColumns: `repeat(${columns}, ${cell}px)`, gridAutoRows: `${cell}px` }} aria-hidden="true">
