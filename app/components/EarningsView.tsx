...
export function EarningsView({ state, setState }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState | null>> }) {
  ...
  return <section className="earnings-shell">
    ...
    <section className="panel investment-panel"><span className="eyebrow">OPTIONAL INVESTING</span><h2>Risk can make the counter flip direction</h2><p className="muted">These are fictional game investments, not real investment recommendations. In Risk Mode, losses can push you into debt and bankruptcy.</p>{lastResult ? <div className="investment-result">{lastResult}</div> : null}<div className="investment-grid">{investments.map((investment) => { const unlocked = investmentUnlocked(state, investment); return <article key={investment.id}><span>{investment.emoji}</span><div><b>{investment.name}</b><small>{investment.description}</small><em>Outcome range {Math.round(investment.minReturn * 100)}% to +{Math.round(investment.maxReturn * 100)}%</em></div><div>{[0.1,0.25,0.5].map((fraction) => <button key={fraction} disabled={!unlocked || state.runStatus !== 'active'} onClick={() => setState((current) => { if (!current) return current; const result = executeInvestment(current, investment, fraction, worth); if (!result.stake) { setLastResult(`Need at least ${money(investment.minimumStake)} available.`); return current; } setLastResult(`${investment.name}: ${money(result.stake)} at risk → ${result.delta >= 0 ? '+' : ''}${money(result.delta)} (${Math.round(result.returnRate * 100)}%).`); return result.state; })}>{Math.round(fraction * 100)}%</button>)}</div></article>; })}</div></section>
  </section>;
}
