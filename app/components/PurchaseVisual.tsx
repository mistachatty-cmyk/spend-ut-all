...
export function PurchaseVisual({ id, name, emoji = '◆', family, value = 0, tier: explicitTier, rarity, compact = false, pack = false, locked = false, imageSrc, imageAlt, className = '' }: PurchaseVisualProps) {
  ...
  return <div className={`purchase-visual purchase-visual--${family} purchase-visual--tier-${tier} ${compact ? 'purchase-visual--compact' : ''} ${pack ? 'purchase-visual--pack' : ''} ${locked ? 'purchase-visual--locked' : ''} ${className}`} style={style} role="img" aria-label={imageAlt ?? `${name} digital preview, ${gradeNames[tier].toLowerCase()} visual grade`} title={`${name} · ${gradeNames[tier].toLowerCase()} visual grade`}>
    <>
      ...
      <span className="purchase-visual__object" aria-hidden="true">{locked ? '◈' : <PixelPurchaseArt id={id} name={name} family={family} tier={tier} />}</span>
