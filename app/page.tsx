...
import { activeMarketEvent, advance, applyOfflineProgress, buyItem, buyUpgrade, canBuyItem, canBuyUpgrade, chooseCitySpecialization, grossIncomePerSecond, holdingsValue, itemBulkPrice, maxAffordableQuantity, netWorth, newGame, passiveCashPerSecond, scenarioProgress, sellItem, totalOwned, upkeepPerSecond, upgradeCost, upgradeHouse, upgradeRegion, upgradeTown, upgradesValue } from '@/game/engine';
...
type View = 'market' | 'earnings' | 'businesses' | 'empire' | 'achievements' | 'collection' | 'leaderboard';
...
export default function Home() {
  ...
  return <main className={state.theme === 'midnight' ? 'app midnight' : 'app'}>
    <header className="topbar"><div><div className="eyebrow">SPEND IT ALL {state.riskMode ? '· RISK MODE' : ''} {meta.equippedTitle ? `· ${meta.equippedTitle.toUpperCase()}` : ''}</div><MoneyCounter state={state} income={income} /></div><div className="top-stats"><div><span>Net worth</span><b>{money(worth)}</b></div><div><span>Spent</span><b>{money(state.totalSpent)}</b></div><div><span>Achievements</span><b>{achievementCount}/{achievements.length}</b></div><div><span>LOK</span><b>◈ {state.lokTokens.toLocaleString()}</b><small>+1 in {Math.max(1, Math.ceil((10_000 - state.lokProgressMs) / 1000))}s</small></div></div></header>
