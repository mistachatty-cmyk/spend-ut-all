...
import { advanceWithDebt, canSellItemWithDebt, leveragedNetWorth, scenarioProgressWithDebt, sellItemWithDebt } from '@/game/debt-runtime';
...
export default function Home() {
  ...
  useEffect(() => { if (!state || !metaLoaded) return; setMeta((current) => syncMetaProgression(current, state, { netWorth: leveragedNetWorth(state), incomePerSecond: passiveCashPerSecond(state), totalOwned: totalOwned(state), scenarioComplete: scenarioProgressWithDebt(state) >= 1 })); }, [state, metaLoaded]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!state?.started || state.runStatus !== 'active' || recordedWin.current === state.createdAt || scenarioIsFreeMode(state) || scenarioProgressWithDebt(state) < 1) return;
  ...
  useEffect(() => { if (!state?.started || state.runStatus !== 'active') return; let last = performance.now(); const timer = window.setInterval(() => { const now = performance.now(); const delta = Math.min(now - last, 5_000); last = now; setState((current) => current ? advanceWithDebt(current, delta) : current); }, 250); return () => window.clearInterval(timer); }, [state?.started, state?.runStatus]);
  ...
  const income = passiveCashPerSecond(state), gross = grossIncomePerSecond(state), upkeep = upkeepPerSecond(state), worth = leveragedNetWorth(state), holdings = holdingsValue(state), upgradeValue = upgradesValue(state);
  const progress = scenarioProgressWithDebt(state), freeMode = scenarioIsFreeMode(state), won = !freeMode && progress >= 1, goalLabel = scenarioGoalLabel(state);
  ...
  return <main className={appClass}>
    ...
    {view === 'market' ? <><nav className="tabs">{['all','everyday','luxury','property','business','infrastructure'].map((entry) => <button key={entry} className={category === entry ? 'active' : ''} onClick={() => setCategory(entry)}>{entry}</button>)}</nav><section className="dashboard-grid"><section className="catalog panel"><div className="section-heading"><div><span className="eyebrow">MARKETPLACE</span><h2>Buy the world</h2></div><span>{state.riskMode ? 'Credit enabled · bankruptcy possible' : state.mode === 'advanced' ? 'Revenue + upkeep active' : 'Simple economy'}</span></div><div className="item-list">{visibleItems.map((item) => { const owned = state.owned[item.id] ?? 0, unlocked = state.totalSpent >= (item.unlockSpent ?? 0), price = itemBulkPrice(item, owned, 1, state), maxQty = maxAffordableQuantity(state,item), pledged = isItemPledged(debtState, item.id); return <article className={`item-card ${!unlocked ? 'locked' : ''}`} key={item.id}><div className="item-icon">{item.emoji}</div><div className="item-copy"><div className="item-title"><h3>{item.name}</h3><span>Owned {owned.toLocaleString()}{pledged ? ' · 🔒 pledged' : ''}</span></div><p>{item.description}</p><div className="item-meta"><b>{money(price)}</b>{item.incomePerSecond ? <span>+{money(item.incomePerSecond)}/sec ea.</span> : null}{state.mode === 'advanced' && item.upkeepPerSecond ? <span className="negative">-{money(item.upkeepPerSecond)}/sec upkeep</span> : null}</div></div><div className="buy-stack">{unlocked ? <><button disabled={!canBuyItem(state,item,1)} onClick={() => setState((s) => s ? buyItem(s,item,1) : s)}>Buy</button><button disabled={!canBuyItem(state,item,10)} onClick={() => setState((s) => s ? buyItem(s,item,10) : s)}>×10</button><button disabled={!canBuyItem(state,item,100)} onClick={() => setState((s) => s ? buyItem(s,item,100) : s)}>×100</button><button disabled={maxQty < 1} onClick={() => setState((s) => s ? buyItem(s,item,maxAffordableQuantity(s,item)) : s)}>MAX</button><button className="sell" disabled={!canSellItemWithDebt(state,item)} onClick={() => setState((s) => s ? sellItemWithDebt(s,item,1) : s)}>{pledged ? 'Pledged' : state.scenarioId === 'custom' && state.customScenario && !state.customScenario.restrictions.sellingEnabled ? 'No Sell' : 'Sell'}</button></> : <span>Unlock after {money(item.unlockSpent ?? 0)} spent</span>}</div></article>; })}</div></section><aside className="side-stack"><FinancialPanel state={state} gross={gross} upkeep={upkeep} income={income} holdings={holdings} upgradeValue={upgradeValue} ownedCount={ownedCount} onMode={() => setState((s) => s && !(s.scenarioId === 'custom' && s.customScenario?.rulesLocked) ? {...s, mode: s.mode === 'simple' ? 'advanced' : 'simple'} : s)} /><ProgressPanels state={state} currentHouse={currentHouse} nextHouse={nextHouse} currentTown={currentTown} nextTown={nextTown} currentRegion={currentRegion} nextRegion={nextRegion} setState={setState} /></aside></section></> : null}
    ...
    <footer className="game-footer"><button onClick={() => setCustomization((current) => saveCustomizationInventory(equipCustomization(current, darkTheme ? 'theme-classic-ledger' : 'theme-midnight')))}>Toggle {darkTheme ? 'Classic' : 'Midnight'}</button><button className="secondary" onClick={() => setView('customize')}>Customize ◈</button><button className="secondary" onClick={() => setView('debt')}>Debt & Court ⚖</button><button className="secondary" onClick={() => setView('settings')}>Game Rules ⚙</button><button className="danger" onClick={() => { localStorage.removeItem(SAVE_KEY); setState(null); setOfflineAward(0); }}>Reset Run</button></footer>
  </main>;
}
