...
type View = 'market' | 'earnings' | 'businesses' | 'empire' | 'achievements' | 'collection' | 'leaderboard' | 'settings';
...
export default function Home() {
  ...
  return <main className={state.theme === 'midnight' ? 'app midnight' : 'app'}>
    <header className="topbar"><div><div className="eyebrow">SPEND IT ALL {state.riskMode ? '· RISK MODE' : ''} · {state.rules.presetId.toUpperCase()} {meta.equippedTitle ? `· ${meta.equippedTitle.toUpperCase()}` : ''}</div><MoneyCounter state={state} income={income} /></div><div className="top-stats"><div><span>Net worth</span><b>{money(worth)}</b></div><div><span>Spent</span><b>{money(state.totalSpent)}</b></div><div><span>Achievements</span><b>{achievementCount}/{achievements.length}</b></div><div><span>LOK</span><b>◈ {state.lokTokens.toLocaleString()}</b><small>+1 in {Math.max(1, Math.ceil((10_000 - state.lokProgressMs) / 1000))}s</small></div></div></header>
    ...
    <footer className="game-footer"><button onClick={() => setState((s) => s ? {...s, theme: s.theme === 'light' ? 'midnight' : 'light'} : s)}>Toggle {state.theme === 'light' ? 'Midnight' : 'Light'}</button><button className="secondary" onClick={() => setView('settings')}>Game Rules ⚙</button><button className="danger" onClick={() => { localStorage.removeItem(SAVE_KEY); setState(null); setOfflineAward(0); }}>Reset Run</button></footer>
  </main>;
}
