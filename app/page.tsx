...
import { activeMarketEvent, advance, applyOfflineProgress, buyItem, buyUpgrade, canBuyItem, canBuyUpgrade, chooseCitySpecialization, grossIncomePerSecond, holdingsValue, itemBulkPrice, maxAffordableQuantity, netWorth, newGame, passiveCashPerSecond, scenarioProgress, sellItem, totalOwned, unlockedAchievements, upkeepPerSecond, upgradeCost, upgradeHouse, upgradeRegion, upgradeTown, upgradesValue } from '@/game/engine';
...
type View = 'market' | 'earnings' | 'businesses' | 'empire' | 'collection' | 'leaderboard';
...
export default function Home() {
  ...
  useEffect(() => {
    if (!state?.started || state.runStatus !== 'active' || recordedWin.current === state.createdAt) return;
    const scenario = scenarios.find((entry) => entry.id === state.scenarioId);
    if (!scenario || scenario.freeMode || scenarioProgress(state) < 1) return;
  ...
  if (!state?.started) return <main className="menu-shell"><section className="hero-card"><div className="eyebrow">ECONOMIC EMPIRE SANDBOX · LOK READY</div><h1>Spend It All</h1><p className="lead">Start from absolutely nothing, multiply a small fortune 10× to 1,000×, play with no finish line, or begin absurdly rich. Watch the counter grow while achievements track how you built it.</p><div className="choice-grid">{scenarios.map((scenario) => <button key={scenario.id} className={`choice ${scenarioId === scenario.id ? 'selected' : ''}`} onClick={() => setScenarioId(scenario.id)}><strong>{scenario.name}</strong><span>{scenario.description}</span><b>{money(scenario.startingCash)}</b><small>{scenario.goalLabel}</small></button>)}</div><div className="mode-row"><button className={mode === 'simple' ? 'pill selected' : 'pill'} onClick={() => setMode('simple')}>Simple Financials</button><button className={mode === 'advanced' ? 'pill selected' : 'pill'} onClick={() => setMode('advanced')}>Advanced Financials</button></div><label className="risk-toggle"><input type="checkbox" checked={riskMode} onChange={(event) => setRiskMode(event.target.checked)} /><div><b>Optional Risk Mode</b><small>Allows bounded overspending, debt interest, risky investments, a bankruptcy countdown and a true lost run.</small></div></label><button className="primary" onClick={() => { setOfflineAward(0); setView(scenarioId === 'nothing' ? 'earnings' : 'market'); recordedWin.current = null; setState(newGame(scenarioId, mode, riskMode)); }}>Start Scenario</button><p className="micro">Start From Nothing begins at exactly $0, so your first move is earning. LOK and Legacy Collection persist across every mode.</p></section></main>;
  ...
  const scenario = scenarios.find((entry) => entry.id === state.scenarioId) ?? scenarios[0], progress = scenarioProgress(state), won = !scenario.freeMode && progress >= 1;
  ...
  const earned = unlockedAchievements(state), ownedCount = totalOwned(state), event = activeMarketEvent(state), specialization = citySpecializations.find((entry) => entry.id === state.citySpecialization);

  return <main className={state.theme === 'midnight' ? 'app midnight' : 'app'}>
    <header className="topbar"><div><div className="eyebrow">SPEND IT ALL {state.riskMode ? '· RISK MODE' : ''} {meta.equippedTitle ? `· ${meta.equippedTitle.toUpperCase()}` : ''}</div><MoneyCounter state={state} income={income} /></div><div className="top-stats"><div><span>Net worth</span><b>{money(worth)}</b></div><div><span>Spent</span><b>{money(state.totalSpent)}</b></div><div><span>Legacy</span><b>{meta.badges.length} badges · {meta.collectibles.length} relics</b></div><div><span>LOK</span><b>◈ {state.lokTokens.toLocaleString()}</b><small>+1 in {Math.max(1, Math.ceil((10_000 - state.lokProgressMs) / 1000))}s</small></div></div></header>
    ...
    <section className={`goal-strip ${won ? 'won' : ''}`}><div><span className="eyebrow">SCENARIO GOAL</span><b>{scenario.freeMode ? 'Free Mode ∞ — no finish line' : won ? 'Goal complete ✓ — saved to leaderboard' : scenario.goalLabel}</b></div><div className="goal-progress"><span style={{ width: `${scenario.freeMode ? 100 : progress * 100}%` }} /></div><small>{scenario.freeMode ? '∞' : `${Math.round(progress * 100)}%`}</small></section>
    <nav className="view-tabs"><button className={view === 'market' ? 'active' : ''} onClick={() => setView('market')}>Marketplace</button><button className={view === 'earnings' ? 'active' : ''} onClick={() => setView('earnings')}>Earn</button><button className={view === 'businesses' ? 'active' : ''} onClick={() => setView('businesses')}>Businesses</button><button className={view === 'empire' ? 'active' : ''} onClick={() => setView('empire')}>Empire</button><button className={view === 'collection' ? 'active' : ''} onClick={() => setView('collection')}>Collection · {meta.collectibles.length}</button><button className={view === 'leaderboard' ? 'active' : ''} onClick={() => setView('leaderboard')}>Leaderboard</button></nav>
